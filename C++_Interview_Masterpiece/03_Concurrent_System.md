...
2. [锁机制全景：互斥锁、自选锁、读写锁、条件变量](#2-锁机制全景互斥锁自旋锁读写锁条件变量)
3. [原子操作与内存模型](#3-原子操作与内存模型)
4. [无锁编程：CAS、ABA 问题与内存回收](#4-无锁编程casaba-问题与内存回收)
5. [线程池：设计、参数、优雅关闭](#5-线程池设计参数优雅关闭)
6. [多线程死锁排查](#6-多线程死锁排查)
7. [生产者-消费者模型](#7-生产者-消费者模型)
8. [单例模式的多线程安全](#8-单例模式的多线程安全)
9. [Linux I/O 多路复用：select / poll / epoll](#9-linux-io-多路复用select--poll--epoll)
10. [Reactor 模式与高性能网络框架](#10-reactor-模式与高性能网络框架)
...
### 1.3 async / future / promise / packaged_task
...
> **回答**：`thread_local` 变量在**其所在线程启动时初始化**（线程专用的静态存储期），在**线程结束时析构**。同一程序的不同线程拥有该变量的独立副本，互不影响。它类似于"每个线程的 static 变量"，典型用途：线程局部缓存、per-thread logger、rand 种子。
...
### 3.3 C++ Memory Order：全部 6 种

```
   最强保证（代价最高）
   ↓
┌─────────────────────────────────────────┐
│ memory_order_seq_cst │ 全局顺序一致性      │ 最安全，最慢（默认值）
├─────────────────────────────────────────┤
│ memory_order_acq_rel │ acquire + release │ 适合 CAS 操作
├──────────────────────┼───────────────────┤
│ memory_order_acquire │ 读侧栅栏           │ 之后的读写不能重排到前面
├──────────────────────┼───────────────────┤
│ memory_order_release │ 写侧栅栏           │ 之前的读写不能重排到后面
├──────────────────────┼───────────────────┤
│ memory_order_consume │ 数据依赖排序        │ C++17 起不推荐，通常 = acquire
├──────────────────────┼───────────────────┤
│ memory_order_relaxed │ 仅保证原子性        │ 最快，不保证顺序
└─────────────────────────────────────────┘
   最弱保证（代价最低）
   ↓
```
...
void onRequest() {
    hit_count.fetch_add(1, std::memory_order_relaxed);  // 不关心和其他操作的顺序
}
...
> **回答**：当**只关心原子性，不关心与其他操作的顺序**时。典型场景：全局计数器（如请求计数）、引用计数的原子增减（`shared_ptr` 内部）、统计信息收集。relaxed 在 x86 上几乎无额外开销（x86 本身就提供较强的硬件内存序），在 ARM/PowerPC 等弱内存序平台上可以避免昂贵的内存屏障指令。
...
## 4. 无锁编程：CAS、ABA 问题与内存回收

### 4.1 无锁栈 (Treiber Stack) — 最简无锁原型
...
```cpp
// Tagged Pointer 方案（128-bit CAS，x86-64 可用 cmpxchg16b）
template<typename T>
struct TaggedPtr {
    T* ptr;
    uintptr_t tag;  // 版本号
    ...
    bool CAS(TaggedPtr& expected, TaggedPtr desired) {
        // 使用 __atomic_compare_exchange 做 128-bit CAS
        __int128 old_val = pack(expected);
        __int128 new_val = pack(desired);
        return __sync_bool_compare_and_swap_16(
            reinterpret_cast<volatile __int128*>(this), old_val, new_val);
    }
};
```
...
> **回答**：Hazard Pointer 不防止 ABA 发生——它解决的是 ABA 导致的 **use-after-free** 问题。原理：每个线程维护一组"危险指针"，当线程要访问一个节点时，先将节点地址写入自己的 Hazard Pointer 列表。内存在释放前检查所有线程的 Hazard Pointer 列表，如果发现有人正在使用，就延迟释放。这样即使 ABA 导致 CAS 成功，被重用的地址也不会指向已释放的内存。
...
> **回答**：C++11 标准规定：如果多个线程同时进入同一个 `static` 局部变量的初始化，只有一个线程执行初始化，其他线程阻塞等待初始化完成。编译器通过隐藏的原子 flag + mutex 来实现（类似双检锁但由编译器生成）。注意仅保证"初始化"是线程安全的，后续的并发访问仍需自行同步。
...
## 9. Linux I/O 多路复用：select / poll / epoll
