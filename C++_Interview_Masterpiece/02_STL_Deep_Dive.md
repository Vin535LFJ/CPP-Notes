...
> **定位**：容器、迭代器、算法、分配器——底层源码级分析 + 迭代器失效全景 + 内存池设计的哲学。
...
9. [std::sort：内省排序的混合智慧](#9-stdsort内省排序的混合智慧)
10. [手撕常用 STL 组件](#10-手撕常用-stl-组件)
...
| 阶段 | 条件 | 算法 | 复杂度 |
|------|------|------|--------|
| 主力 | 常规区间 | 快速排序（三数取中 pivot） | O(n log n) 平均 |
| 兜底 | 递归深度 > 2*log₂(n) | 堆排序 | O(n log n) 最坏 |
| 尾声 | size < 16 | 插入排序 | O(n²) 最坏但常数极小 |
